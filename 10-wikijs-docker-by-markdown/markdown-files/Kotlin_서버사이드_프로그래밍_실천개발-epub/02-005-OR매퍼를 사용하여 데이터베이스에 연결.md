...
명령 5.2.5
 ...
 mysql> use example;
 Database changed
그리고 Listing 5.2.6 의 Create 문으로 테이블을 생성한다.
...
명령 5.2.7
마지막으로 Listing 5.2.8 의 쿼리에서 샘플 데이터를 등록한다.
...
build.gradle.kts에 몇 가지 설명을 추가합니다. 먼저 plugins 블록에 Listing 5.3.1 의 행을 추가한다.

Listing 5.3.1

 id("com.arenagod.gradle.MybatisGenerator") version "1.4"
이것은 나중에 설명하는 MyBatis의 코드 생성에 사용되는 Gradle 플러그인을 추가합니다. 그런 다음 dependencies 블록에 Listing 5.3.2 의 4행을 추가한다.
...
그리고 파일 끝에 Listing 5.3.3 의 블록을 추가한다.
...
build.gradle.kts의 전체 파일은 Listing 5.3.4 와 같다.
...
구성 파일 추가
프로젝트의 src/main/resources 아래에 generatorConfig.xml이라는 이름으로 Listing 5.3.5 의 내용을 가진 파일을 생성한다.
...
울사용자 기록.kt
울UserDynamicSqlSupport.kt
울UserMapper.kt
울UserMapperExtensions.kt
각각을 간단히 설명합니다.
...
사전 준비가 길어졌습니다만, 드디어 MyBatis를 사용한 데이터베이스에 액세스하는 CRUD(Create=Insert, Read=Select, Update, Delete)를 구현합니다. 그 전에는 다른 구성 파일이 필요하고 src/main/resources 아래에 mybatis-config.xml이라는 이름으로 Listing 5.4.1 의 내용을 가진 파일을 생성한다.
...
그리고 모든 Kotlin 파일을 만들고 Listing 5.4.2 의 함수를 작성한다.
...
기본 키 검색
Select 문을 실행하는 프로세스는 몇 가지 패턴이 있지만, 우선은 가장 간단한 기본 키 검색 처리부터입니다. Listing 5.4.3 을 보라.
...
Where 절에서 검색
다음은 기본 키 이외의 열을 조건으로 지정한 검색입니다. Listing 5.4.4 와 같이 구현하면 where 절에서 검색 조건을 지정할 수 있다.
...
실행 쿼리는 Listing 5.4.5 와 동일하다.

Listing 5.4.5

 select id, name, age, profile from user where name = "Jiro";
다른 조건 지정도 시도합니다. Listing 5.4.6 을 보라.
...
쿼리에서는 Listing 5.4.7 이 동일하다.

Listing 5.4.7

 select id, name, age, profile from user where age >= 25;
count 사용
Select 문에서 또 하나 count 를 소개합니다. SQL의 count 함수에 해당하는 처리가 됩니다. Listing 5.4.8 을 보라.
...
또한 where 의 조건으로 Listing 5.4.9 와 같이 allRows 라는 함수를 사용하면 모든 레코드를 대상으로 할 수 있습니다.
...
> 실행 결과
 ...
 1行のレコードを挿入しました
Select와 마찬가지로 Mapper 객체를 가져 와서 UserMapperExtensions.kt의 insert 함수를 실행하고 있습니다. insert 의 인수는 대상 테이블의 xxxxRecord 형 (여기에서는 UserRecord 형)의 오브젝트로, 등록하고 싶은 데이터의 내용을 설정한 인스턴스를 생성해, 인수에 건네줍니다. 그리고 반환값에는, 등록한 건수가 돌아오기 때문에, 실행 결과에 출력하고 있습니다. Listing 5.4.10 에는 import 문이 포함되어 있지만, 이 import 문을 쓰지 않으면 UserMapper 인터페이스의 insert 함수가 호출되어 오류가 발생하기 때문에 필요하다. 이 장에서는 마찬가지로 import문을 기재한 샘플이 몇 가지 나오므로, 마찬가지로 import문을 수동으로 추기해 주세요.
...
명령 5.4.11
다중 레코드 Insert
Listing 5.4.12 와 같이 insertMultiple 을 사용하여 여러 레코드를 함께 Insert할 수 있다.
...
Record 객체를 사용하지 않고 Listing 5.4.16 과 같이 업데이트할 수도 있다.
...
기본 키 이외의 열을 조건으로 한 업데이트에서도 Record 객체를 사용할 수 있습니다. Listing 5.4.18 을 보라.
...
dependencies 블록에 Listing 5.5.3 종속성을 추가한다.
...
구성 파일 추가
데이터베이스의 접속처 정보 등을 쓴 설정 파일도 마찬가지로 추가합니다. src/main/resources 아래에 Listing 5.5.4 의 내용으로 generatorConfig.xml을 생성한다.
...
application.yml은 Spring Framework에서 사용하는 설정을 설명하는 파일입니다. mybatis-spring-boot-starter 를 사용하고 있기 때문에 Listing 5.4.1 에서 작성한 데이터 소스 관련 설정도 여기에 쓸 수 있습니다.
...
com.example.demo 패키지 아래에 Listing 5.5.6 의 Controller 클래스를 생성한다.
...
Insert API 추가
또 다른 API를 만들어 보겠습니다. 이번에는 user 테이블에 Insert를 실행하는 API입니다. 등록 처리이므로 POST 메소드의 API를 작성합니다. 요청, 응답 클래스는 Listing 5.5.8 과 같이 생성된다.
...
그리고 UserController 클래스에 Listing 5.5.9 의 함수를 추가한다.
...
注1 　 https://docs.docker.com/get-docker/
