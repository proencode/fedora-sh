...
do
	#-- kaos server 로컬 디렉토리
	#-- /var/base/cadbase/2023/02/15
	server_var_base_basename_Y4M2D2=/var/base/${base_name}/${y4}/${m2}/${d2}

	#-- 클라우드쪽 폴더
	#-- kaosdb/2023/cadbase/02
	cloud_kaosdb_Y4_basename_M2=kaosdb/${y4}/${base_name}/${m2}

	#-- crontab 쪽 디렉토리
	#-- /opt/backup/kaosdb/2023/cadbase/02
	local_opt_backup_kaosdb_Y4_basename_m2=/opt/backup/${cloud_kaosdb_Y4_basename_M2}
	...
	cmdRun "rm -rf ${local_opt_backup_kaosdb_Y4_basename_m2}/${y2}${m2}${d2}-${base_name}*" "(2) 이전에 있던 지정일자 파일을 지웁니다."
	...
	if [ ! -d ${local_opt_backup_kaosdb_Y4_basename_m2}/${d2} ]; then
		...
		local_lsalR="${y2}${m2}${d2}-${base_name}-${ymdHMS}.ls-alR"
		cmdRun "ls -alR ${local_opt_backup_kaosdb_Y4_basename_m2}/${d2}/ > ${local_opt_backup_kaosdb_Y4_basename_m2}/${local_lsalR}" "(5) 목록을 만들고, 7za a -mx=9 -p ${local_opt_backup_kaosdb_Y4_basename_m2} 디렉토리를 압축합니다.${cReset}"
		time 7za a -mx=9 -p${pscode} ${local_opt_backup_kaosdb_Y4_basename_m2}/${y2}${m2}${d2}-${base_name}-${ymdHMS}.7z ${local_opt_backup_kaosdb_Y4_basename_m2}/${d2}
		cmdRun "rm -rf ${local_opt_backup_kaosdb_Y4_basename_m2}/${d2}" "(6) 압축했으므로 백업한 디렉토리는 지웁니다."

		cmdRun "rclone delete kaosngc:${cloud_kaosdb_Y4_basename_M2} --include \"${y2}${m2}${d2}-${base_name}*\"" "(7) 복사하기전 클라우드에 동일 날짜 데이터를 삭제합니다."
		cmdRun "rclone copy ${local_opt_backup_kaosdb_Y4_basename_m2} --include \"${y2}${m2}${d2}-${base_name}*\" kaosngc:${cloud_kaosdb_Y4_basename_M2}" "(8) 로컬 파일들을 kaosngc: 클라우드로 복사합니다."
		cmdRun "rclone ls kaosngc:${cloud_kaosdb_Y4_basename_M2}" "(9) 클라우드 확인"
	fi
done


base_name=kaosorder2

#-- 이 폴더의 base_db 이름이 달라서 위 for 문에 넣지 못했음.
#-- kaos server 로컬 디렉토리
#-- /var/base_db/kaosorder2/2023/02
server_var_base_db_basename_Y4_M2=/var/base_db/${base_name}/${y4}/${m2}

#-- 클라우드쪽 폴더
#-- kaosdb/2023/kaosorder2/02
cloud_kaosdb_Y4_basename_M2=kaosdb/${y4}/${base_name}/${m2}

#-- crontab 쪽 디렉토리
#-- /opt/backup/kaosdb/2023/kaosorder2/02
local_opt_backup_kaosdb_Y4_basename_m2=/opt/backup/${cloud_kaosdb_Y4_basename_M2}

if [ ! -d "${local_opt_backup_kaosdb_Y4_basename_m2}" ]; then
	cmdRun "mkdir -p ${local_opt_backup_kaosdb_Y4_basename_m2} ; ls -l ${local_opt_backup_kaosdb_Y4_basename_m2}/../" "(11) 백업할 폴더를 만듭니다."
fi
cmdRun "rm -rf ${local_opt_backup_kaosdb_Y4_basename_m2}/${base_name}_${y2}${m2}${d2}*" "(12) 이전에 있던 지정일자 파일을 지웁니다."

if [ "x${prompt_ok}" = "ok" ]; then
	echo "${cCyan}#-- time rsync -avzr --delete --rsh=\"/usr/bin/sshpass -f ${HOME}/.ssh/kaosco.4ssh ssh -oHostKeyAlgorithms=+ssh-dss -Y -p2022 -o StrictHostKeyChecking=no -l kaosco\" --no-o --no-g --delete kaosco@kaos.kr:${server_var_base_db_basename_Y4_M2}/${base_name}_${y2}${m2}${d2}* ${local_opt_backup_kaosdb_Y4_basename_m2}/ ${cGreen}#-- (13) ${cMagenta}호스트에서 로컬로 백업파일을 복사합니다.${cReset}"
fi
time rsync -avzr --delete --rsh="/usr/bin/sshpass -f ${HOME}/.ssh/kaosco.4ssh ssh -oHostKeyAlgorithms=+ssh-dss -Y -p2022 -o StrictHostKeyChecking=no -l kaosco" --no-o --no-g --delete kaosco@kaos.kr:${server_var_base_db_basename_Y4_M2}/${base_name}_${y2}${m2}${d2}* ${local_opt_backup_kaosdb_Y4_basename_m2}/

cmdRun "rclone delete kaosngc:${cloud_kaosdb_Y4_basename_M2} --include \"${base_name}_${y2}${m2}${d2}*\"" "(15) 복사하기전 클라우드에 동일 날짜 데이터를 삭제합니다."
cmdRun "rclone copy ${local_opt_backup_kaosdb_Y4_basename_m2} --include \"${base_name}_${y2}${m2}${d2}*\" kaosngc:${cloud_kaosdb_Y4_basename_M2}" "(16) 로컬 파일들을 kaosngc: 클라우드로 복사합니다."
