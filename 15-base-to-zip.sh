	...
	do
		...
		do
			...
			if [ -d ${base_year_mm} ]; then
				psco="zkdhtm${year}"
				echo "$(date +'%H:%M:%S:')#----> ls -alR ${base_year_mm} > ${base}/${year}/${base}-${year}-${mm}.ls-alR" >> zip-log
				ls -alR ${base_year_mm} > ${base}/${year}/${base}-${year}-${mm}.ls-alR
				echo "$(date +'%H:%M:%S:')#----> 7za a -mx=9 -p${psco} ${base}/${year}/${base}-${year}-${mm}.7z ${base}/${year}/${mm}" >> zip-log
