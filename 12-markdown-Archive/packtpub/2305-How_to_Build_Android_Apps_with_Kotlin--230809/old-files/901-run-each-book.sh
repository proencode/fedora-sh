...
if [ "x$4" != "x" ]; then
	...
	ChapterSeq="01.c1" #-- 01=권 일련번호 c1=Section/Chapter 번호
fi
if [ "x$5" != "x" ]; then
	ChapterName="$5"
else
...
${cGreen}----> (1) ${cCyan}폴더 이름으로 쓰기 위한 책 제목 Title: ${cRed}[ ${cGreen}${BookCover} ${cRed}] ${cMagenta}= 알파벳만 = ${cBlue}(${cMagenta}대/소 문자 숫자 '.' '-' '_' 빈칸${cBlue})${cReset}
__EOF__
read a
if [ "x$a" = "x" ]; then
	echo "${cUp}"
...
book_jemok=$(echo "${BookCover,,}" | sed 's/ /_/g') #-- 책 제목: 소문자로 바꾸고 공백을 밑줄로 바꾼다.

cat <<__EOF__

${cBlue}====> ${cMagenta}[ ${cGreen}${chulpansa} ${cBlue}/ ${cGreen}${book_jemok} ${cBlue}책 제목: ${cGreen}${BookCover} ${cMagenta}] ${cBlue}책 제목을 정했습니다.${cReset}
__EOF__
read a
if [ "x$a" = "x" ]; then
	echo "${cUp}"
...
cat <<__EOF__
      ${cRed}[ ${cYellow}${chulpansa} ${cBlue}/ ${cYellow}${book_jemok} ${cBlue}책 제목: ${cYellow}${BookCover} ${cRed}]
...
> Path: ${chulpansa}/${book_jemok}/${ChapterSeq}_${chapter_name}
> tags: ${tags}
> this File Name: ${thisFileName}.md

> Chapter Name: ${ChapterSeq} ${ChapterName}
> Link: ${https_line}
> Images: / ${chulpansa} / ${book_jemok} /
> create: $(date +'%Y-%m-%d %a %H:%M:%S')

# ${ChapterSeq} ${ChapterName}


![ ${image_jemok} ]( /${chulpansa}/${book_jemok}_img/${img_name}.webp
)
${cBlue}
/ / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / /
${cBlue}
