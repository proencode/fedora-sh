...
fedora-sh $ echo "#-- df 의 /마운트별 사이즈와 그 최종합계" ; df | grep -v "d A" | while read fsys siz usd avl usp mton ; do tt="$((tt + siz))" ; mega=$((tt / 1000)) ; printf "%'12.f ${mton} ________ (tot: %'.f M)" $siz $mega ; echo "" ; done #-- Filesystem(fsys) 1K-blocks(siz) Used(usd) Available(avl) Use%(usp) Mounted on(mton)
#-- df 의 /마운트별 사이즈와 그 최종합계
...
do
    ...
    echo ""
done #-- Filesystem(fsys) 1K-blocks(siz) Used(usd) Available(avl) Use%(usp) Mounted on(mton)
```
...
services:
  ...
  db:
    ...
    environment:
      ...
      POSTGRES_PASSWORD: wikipswd #-- wikijsrocks 비번
    logging:
      driver: "none"
  ...
  wiki:
    ...
    environment:
      ...
      DB_PASS: wikipswd #-- wikijsrocks 비번
    restart: unless-stopped
    ports:
      - "9900:3000"
    container_name:
