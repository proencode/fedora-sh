...
echo "(1-1) dnf 의 플러그인 코어를 받습니다: time sudo dnf -y install dnf-plugins-core real	0m35.843s"
...
echo "(1-5) docker-ce 를 받습니다: time sudo dnf makecache real	0m26.703s"
time sudo dnf makecache
echo "(1-6) time sudo dnf -y install docker-ce docker-ce-cli containerd.io"
...
echo "(1-10) docker-compose 를 설치합니다: time sudo dnf -y install docker-compose #-- real	0m12.520s"
...
echo "(4-2) cd ${YML_DIR} "
cd ${YML_DIR}
echo "(4-3) 도커 컴포즈 설치: time sudo dnf -y install docker-compose"
time sudo dnf -y install docker-compose
echo "(4-4) 설치내역 확인: rpm -qi docker-compose"
rpm -qi docker-compose
echo "(4-5) sudo docker ps -a"
sudo docker ps -a
echo "(4-6) 도커 컴포즈 빌드: time sudo docker-compose pull ${WIKI_SERVICE}"
...
if [ "x${CLOUD_NAME}" = "x" ]; then
    ...
    echo "(5-3) rclone lsf ${CLOUD_NAME}:"
...
__EOF__
    ...
    ls -lR ${LOCAL_Y2M2}
    # echo "#-- (5-7) 오늘 요일이름의 파일을 클라우드로 복사합니다: sudo /usr/bin/rclone copy ${LOCAL_Y2M2}/${current_backup_7z} ${CLOUD_NAME}:${CLOUD_Y2M2}/"
    # sudo /usr/bin/rclone copy ${LOCAL_Y2M2}/${DB_sql7z} ${CLOUD_NAME}:${CLOUD_Y2M2}/
    # echo "#-- (5-8) 클라우드 폴더입니다. /usr/bin/rclone lsl ${CLOUD_NAME}:${CLOUD_Y2M2}"
...
||| "last-${DB_CONTAINER}-\$(date +%y%m%d_%H%M%S)-$(uname -n).sql.7z" 인 경우:
||| 
||| zzz="${LOCAL_Y2M2}/last-${DB_CONTAINER}-\$(date +%y%m%d_%H%M%S)-$(uname -n).sql.7z"
|||
...
(6-8) 지정한 백업파일을 데이터베이스에 붓기 전에,
      ...
      time sudo docker exec ${DB_CONTAINER} pg_dumpall -U ${DB_user_NAME} | 7za a -mx=9 -p -si ${current_backup_7z}
__EOF__
        time sudo docker exec ${DB_CONTAINER} pg_dumpall -U ${DB_user_NAME} | 7za a -mx=9 -p -si ${current_backup_7z}
        cat <<__EOF__
(6-9) 오늘 요일이름의 파일을 클라우드로 복사합니다.
      time sudo /usr/bin/rclone copy ${current_backup_7z} ${CLOUD_NAME}:${CLOUD_Y2M2}/
__EOF__
        time sudo /usr/bin/rclone copy ${current_backup_7z} ${CLOUD_NAME}:${CLOUD_Y2M2}/
        cat <<__EOF__
(6-11) 클라우드 폴더입니다.
       time /usr/bin/rclone lsl ${CLOUD_NAME}:${CLOUD_Y2M2}
__EOF__
        time /usr/bin/rclone lsl ${CLOUD_NAME}:${CLOUD_Y2M2}
...
(6-12) 기존의 데이터베이스를 삭제합니다.
       time sudo docker exec -it ${DB_CONTAINER} dropdb -U ${DB_user_NAME} ${DATABASE_NAME}
__EOF__
    time sudo docker exec -it ${DB_CONTAINER} dropdb -U ${DB_user_NAME} ${DATABASE_NAME}
cat <<__EOF__
(6-13) 빈 데이터베이스를 새로 만듭니다.
       time sudo docker exec -it ${DB_CONTAINER} createdb -U ${DB_user_NAME} ${DATABASE_NAME}
__EOF__
    time sudo docker exec -it ${DB_CONTAINER} createdb -U ${DB_user_NAME} ${DATABASE_NAME}
    cat <<__EOF__
(6-14) 지정한 백업파일을 데이터베이스에 다시 붓습니다. (RESTORE)
       백업할때 입력한 ----비밀번호---- 를 입력하세요.
       time 7za x -so ${zzz} | sudo docker exec -i ${DB_CONTAINER} psql -U ${DB_user_NAME} ${DATABASE_NAME}
__EOF__
    time 7za x -so ${zzz} | sudo docker exec -i ${DB_CONTAINER} psql -U ${DB_user_NAME} ${DATABASE_NAME}
    cat <<__EOF__
