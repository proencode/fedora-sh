...
교보문고 : https://bit.ly/3VoO5N8 예스24 : https://bit.ly/3CXjWgJ 알라딘 : https://bit.ly/3SZW6q1
...
__EOF__
	echo "(1) sleep 1.1 #-- 위 ${NEW_MADE_FILE} 파일은 임시이므로, 1.1초 지연후 LAST_VALUE_SH 파일 이름을 만든다."
	sleep 1.1
	#-- 첫 작업이므로 여기에서 선언할 것은 위에것 뿐이다.
else
	LAST_VALUE_SH="$(ls -ltr ${BIN_VALUE}*.sh | tail -1 | awk '{print $9}')"
	echo "(3) LAST_VALUE_SH [=${LAST_VALUE_SH}]"
	$(sh ${LAST_VALUE_SH}) #-- 여기서 선언을 해서 아래쪽에서 쓸수 있도록 한다.
fi
...
#-- sed -e 's/^ *//g' -e 's/ *$//g'
LAST_VALUE_SH=$(ls -ltr ${BIN_VALUE}*.sh | tail -1 | awk '{print $9}' | sed -e 's/^ *//g' -e 's/ *$//g') #-- ~/bin/value-*.sh 파일중 최근것.
echo "(5) LAST_VALUE_SH [=${LAST_VALUE_SH}]=\$(ls -ltr \${BIN_VALUE}*.sh | tail -1 | awk '{print \$9}' | sed -e 's/^ *//g' -e 's/ *$//g') #-- ~/bin/value-*.sh 파일중 최근것."
tempname=$(grep DOCKER_DIR ${LAST_VALUE_SH} | awk -F"=" '{print $2}' | awk -F"#" '{print $1}') #-- DOCKER_DIR 의 값을 꺼내서 rempname 에 담는다.
echo "(6) \${tempname} [=${tempname}]=\$(grep DOCKER_DIR \${LAST_VALUE_SH} [=${LAST_VALUE_SH}] | awk -F\"=\" '{print \$2}') #-- DOCKER_DIR 의 값을 꺼내서 rempname 에 담는다."

LAST_VALUE_SH=${BIN_VALUE}-$(date +%y%m%d-%H%M%S).sh
cat >> ${LAST_VALUE_SH} <<__EOF__
#-- ${LAST_VALUE_SH} --
DOCKER_DIR=${DOCKER_DIR} #-- dwjs (2-1) 도커 디렉토리 홈
DATABASE_NAME=${DATABASE_NAME} #-- dwjs (3-4) 데이터베이스 이름
DB_user_NAME=${DB_user_NAME} #-- dwjs (3-13) 데이터베이스 유저 이름
...
DOCKER_DIR=/home/docker #-- (2-1) 도커 디렉토리 홈
```
...
#-- #-- 수정전: tempname=$(grep DOCKER_DIR ${LAST_VALUE_SH} | awk -F"=" '{print $2}' | awk -F"#" '{print $1}')
#-- tempname="${DOCKER_DIR}"
#-- if [ "x${tempname}" = "x" ]; then
#--     tempname="/home/docker"
#-- fi
tempname="${DOCKER_DIR}"; if [ "x${tempname}" = "x" ]; then tempname="/home/docker"; fi
echo "🖍️ (2-1) 도커 디렉토리 : [ 그냥 엔터 = ${tempname} ]"
read a; if [ "x${a}" != "x" ]; then tempname=${a}; fi
DOCKER_DIR="$(echo \"${tempname}\" | sed -e 's/^ *//g' -e 's/ *$//g')" #-- 앞뒤 공백 제거
echo "🎶 (2-2) dwjs DOCKER_DIR=\"${DOCKER_DIR}\" #-- 도커 디렉토리"

tempname=${YML_DIR}; if [ "x${tempname}" = "x" ]; then tempname="wikijs_yml"; fi
echo "🖍️ (2-3) docker-compose.yml 을 보관하는 ${DOCKER_DIR} 아래의 디렉토리 : [ 그냥 엔터 = ${tempname} ]"
read a; if [ "x${a}" != "x" ]; then tempname=${a}; fi
YML_DIR="$(echo \"${DOCKER_DIR}/${tempname}\" | sed -e 's/^ *//g' -e 's/ *$//g')" #-- 앞뒤 공백 제거
...
sudo ls -alR ${YML_DIR}
echo "🖍️ (2-7) press Enter:"; read a
echo "🎶 (2-8) dwjs YML_DIR=\"${YML_DIR}\" #-- docker-compose.yml 을 보관하는 ${DOCKER_DIR} 아래의 디렉토리"

tempname=${YML_NAME}; if [ "x${tempname}" = "x" ]; then tempname="docker-compose.yml"; fi
echo "🖍️ (2-9) docker-compose.yml 이름을 지정 : [ 그냥 엔터 = ${tempname} ]"
read a; if [ "x${a}" != "x" ]; then tempname=${a}; fi
YML_NAME="$(echo \"${YML_DIR}/${tempname}\" | sed -e 's/^ *//g' -e 's/ *$//g')" #-- 앞뒤 공백 제거
echo "🎶 (2-10) dwjs YML_NAME=\"${YML_NAME}\" #-- docker-compose.yml 이름을 지정"
```
