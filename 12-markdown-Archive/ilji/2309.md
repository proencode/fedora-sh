...
if [ ! -d ${YML_DIR} ]; then
    echo "(3-1) docker-compose.yml 보관 디렉토리 만들기: sudo mkdir -p ${YML_DIR} ; sudo chmod 755 ${YML_DIR}"
    sudo mkdir -p ${YML_DIR} ; sudo chmod 755 ${YML_DIR}
fi
...
echo "(4-2) cd ${YML_DIR}
cd ${YML_DIR}
echo "(4-3) time sudo dnf -y install docker-compose (설치)"
time sudo dnf -y install docker-compose
...
__EOF__
    echo "time sudo docker exec ${DATABASE_CONTAINER} pg_dumpall -U ${DB_USER} | 7za a -p -si ${LOCAL_Y2M2}/${current_backup_7z}"
    time sudo docker exec ${DATABASE_CONTAINER} pg_dumpall -U ${DB_USER} | 7za a -p -si ${LOCAL_Y2M2}/${current_backup_7z}
...
#==
#== time sudo docker exec ${DB_CONTAINER} pg_dumpall -U ${DB_USER} | 7za a -mx=9 -p -si ${current_backup_7z}
__EOF__
        time sudo docker exec ${DB_CONTAINER} pg_dumpall -U ${DB_USER} | 7za a -mx=9 -p -si ${current_backup_7z}
        cat <<__EOF__
...
#-- time sudo docker exec -it ${DB_CONTAINER} dropdb -U ${DB_USER} ${DB_NAME}
__EOF__
    time sudo docker exec -it ${DB_CONTAINER} dropdb -U ${DB_USER} ${DB_NAME}
cat <<__EOF__
#-- (7-13) 빈 데이터베이스를 새로 만듭니다.
#-- time sudo docker exec -it ${DB_CONTAINER} createdb -U ${DB_USER} ${DB_NAME}
__EOF__
    time sudo docker exec -it ${DB_CONTAINER} createdb -U ${DB_USER} ${DB_NAME}
    cat <<__EOF__
