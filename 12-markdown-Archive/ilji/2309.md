...
2. 시스템을 최신버전으로 유지하기 위해 다음을 실행한다.
```
sudo dnf -y update
```

3. 시스템 명령어인 dnf 의 플러그인 코어를 받는다.
```
sudo dnf -y install dnf-plugins-core
echo "----> (3) --------------------"
```

4. 지금 받으려는 docker-ce 가 있는 위치를 리파지토리에 기록한다.
```
...
## 1. 데이터베이스가 사용하는 디렉토리를 만든다.

```
for i in {0..3}; do echo "#"; done

if [ -d /home/docker/wiki_db ]; then
    echo "----> (1-1) 디렉토리가 있으므로, 내용을 확인하고, ' sudo rm -rf /home/docker/wiki_data ' 명령으로 삭제해야 합니다."
else
    sudo mkdir -p /home/docker/wiki_data
fi
sudo ls -alR /home/docker/wiki_data
echo "----> (1-2) --------------------"
```

## 2. docker-compose.yml 을 보관하는 디렉토리를 만든다.

```
for i in {0..3}; do echo "#"; done

if [ -d /home/docker/wiki_conf ]; then
    echo "----> (2-1) 디렉토리가 있으므로, 내용을 확인하고, ' sudo rm -rf /home/docker/wiki_conf ' 명령으로 삭제해야 합니다."
else
    sudo mkdir -p /home/docker/wiki_conf
fi
sudo ls -alR /home/docker/wiki_data
sudo ls -alR /home/docker/wiki_conf
echo "----> (2-2) --------------------"
```

## 3. 설정파일 docker-compose.yml 만들기

```
for i in {0..9}; do echo "#"; done

DOCKER_DIR="/home/docker"
echo "#----> (3-1) 도커 디렉토리: [ ${DOCKER_DIR} ]"
read a
if [ "x${a}" != "x" ]; then
    DOCKER_DIR=${a}
fi
...
echo "#----> (3-2) 데이터베이스 보관 디렉토리: [ ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
tempname="wikijs_yml"
echo "#----> (3-3) docker-compose.yml 을 보관하는 디렉토리: [ ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
echo "#----> (3-4) 데이터베이스 서비스 이름: [ ${DATABASE_SERVICE} ]"
read a
if [ "x${a}" != "x" ]; then
    DATABASE_SERVICE=${a}
fi
cat <<__EOF__
#-- 데이터베이스 서비스 = [ ${DATABASE_SERVICE} ]

__EOF__

DATABASE_NAME="wikidb"
echo "#----> (3-5) 데이터베이스 이름: [ ${DATABASE_NAME} ]"
read a
if [ "x${a}" != "x" ]; then
    DATABASE_NAME=${a}
fi
...
DATABASE_CONTAINER="pgwikic"
echo "#----> (3-6) 데이터베이스 컨테이너 이름: [ ${DATABASE_CONTAINER} ]"
read a
if [ "x${a}" != "x" ]; then
...
WIKI_SERVICE="wiki_srv"
echo "#----> (3-7) 위키 서비스 이름: [ ${WIKI_SERVICE} ]"
read a
if [ "x${a}" != "x" ]; then
    WIKI_SERVICE=${a}
fi
...
WIKI_PORT_NO="7700"
echo "#----> (3-8) 위키 포트 번호: [ ${WIKI_PORT_NO} ]"
read a
if [ "x${a}" != "x" ]; then
    WIKI_PORT_NO=${a}
fi
cat <<__EOF__
#-- 위키 포트 번호 = [ ${WIKI_PORT_NO} ]

__EOF__

WIKI_CONTAINER="wikijsc"
echo "#----> (3-9) 위키 컨테이너 이름: [ ${WIKI_CONTAINER} ]"
read a
if [ "x${a}" != "x" ]; then
    WIKI_CONTAINER=${a}
fi
cat <<__EOF__
#-- 위키 컨테이너 = [ ${WIKI_CONTAINER} ]
...
echo "#-- (3-10) ls -lR ${YML_NAME}"
ls -lR ${YML_NAME}
echo "#-- (3-11) cat ${YML_NAME}"
cat ${YML_NAME}

if [ ! -d ~/bin ]; then
    mkdir ~/bin
fi
cat > ~/bin/wikijs_name_define.txt <<__EOF__
...
echo ${YML_DIR}/name_define.txt

#-- docker-compose.yml 보관 디렉토리 = [ ${YML_DIR} ]
echo "#-- (4-1) cd /home/docker/wiki_conf"
cd /home/docker/wiki_conf
echo "#-- (4-2) sudo dnf -y install docker-compose (설치)"
sudo dnf -y install docker-compose
echo "#-- (4-3) rpm -qi docker-compose (내역)"
rpm -qi docker-compose
echo "#-- (4-4) sudo docker ps -a"
sudo docker ps -a
echo "#-- (4-5) sudo docker-compose pull wiki_srv (빌드)"
...
echo "#-- (4-7) sudo docker-compose up --force-recreate & (빌드한 도커 컴포즈를 실행합니다)"
sudo docker-compose up --force-recreate &
echo "#-- (4-8) sudo docker-compose ps -a"
...
LOCAL_Y2M2=/home/backup/wikidb_c/${this_y4m2}
if [ ! -d ${LOCAL_Y2M2} ]; then
    echo "#-- (5-1) ${LOCAL_Y2M2} 로컬 디렉토리를 만듭니다."
    mkdir -p ${LOCAL_Y2M2}
fi
CLOUD_NAME=tp3mi
echo "#----> (5-2) rclone 이름: [ ${CLOUD_NAME} ]"
read CLOUD_NAME
if [ "x${CLOUD_NAME} = "x" ]; then
    echo "#----> (5-3) 클라우드 이름이 없어서 중단합니다."
else
    CLOUD_Y2M2=wikidb_c/${this_y4m2} #-- qqq
    echo "#-- (5-4) 위키 도커를 중단합니다."
    echo "#-- sudo docker ps -a ; sudo docker stop wikijs_c"
    sudo docker ps -a ; sudo docker stop wikijs_c
...
#- DB 백업
#-
#- (5-7) wili.js 데이터베이스를 백업하기 위해서 아래에 ---비밀번호--- 를 입력하세요.
#-
__EOF__
    echo "#-- (5-8) 오늘 요일이름의 로컬 보관장소에 백업합니다."
    ...
    echo "#-- (5-9) 보관용 로컬 폴더입니다."
    echo "#-- ls -lR ${LOCAL_Y2M2}"
    ls -lR ${LOCAL_Y2M2}
    # echo "#-- (5-10) 오늘 요일이름의 파일을 클라우드로 복사합니다."
...
hhh=$(tput bold)$(tput setaf 0); rrr=$(tput bold)$(tput setaf 1);
