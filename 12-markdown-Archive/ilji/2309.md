...
LOCAL_FOLDER="${DOCKER_DIR}/backup" #------- dwjs 압축한 데이터베이스 백업 디렉토리
YML_DIR="${DOCKER_DIR}/wikijs_yml" #-------- dwjs docker-compose.yml 을 보관하는 /home/docker/ 아래의 디렉토리
YML_NAME="${DOCKER_DIR}/docker-compose.yml" #-- dwjs docker-compose.yml 이름을 지정
...
LOCAL_Y2M2=${LOCAL_FOLDER}/${DATABASE_CONTAINER}/${this_y4m2}
if [ ! -d ${LOCAL_Y2M2} ]; then
    echo "(5-1) ${LOCAL_Y2M2} 로컬 디렉토리를 만듭니다."
    mkdir -p ${LOCAL_Y2M2}
fi

echo "(5-2) rclone lsf ${CLOUD_NAME}"
rclone lsf ${CLOUD_NAME}

if [ "x${CLOUD_NAME} = "x" ]; then
    echo "(5-3) 클라우드 이름이 없어서 중단합니다."
else
    CLOUD_Y2M2=${DATABASE_CONTAINER}/${this_y4m2}
    echo "(5-4) 위키 도커를 중단합니다."
...
__EOF__
    ...
    echo "(5-9) 위키 도커를 다시 시작합니다: sudo docker start ${WIKI_CONTAINER} ; sudo docker ps -a"
...
if [ "x${zzz}" = "x" ] || [ ! -f "${zzz}" ]; then
    #-- ----> 리스토어 파일이 없으면 여기서 끝냄.
    cat <<__EOF__
||| (6-2) ls -lR ${LOCAL_FOLDER} #--- 백업 디렉토리 내용
$(ls -lR ${LOCAL_FOLDER})
|||
