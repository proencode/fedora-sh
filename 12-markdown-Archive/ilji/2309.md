...
tempname="wikijs_yml"
echo "#----> (2-3) docker-compose.yml 을 보관하는 디렉토리 : [ ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
tempname="docker-compose.yml"
echo "#----> (2-4) docker-compose.yml 이름을 지정 : [ ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
if [ -d ${YML_DIR} ]; then
    cat <<__EOF__
----> (2-7) ${YML_DIR} 디렉토리가 있으므로, 다음 명령으로 내용을 확인하고, 디렉토리  삭제후 새로 만들어야 합니다.
sudo ls -l ${YML_DIR}
...
echo "#----> (2-14) 위키 컨테이너 이름: [ ${WIKI_CONTAINER} ]"
read a
if [ "x${a}" != "x" ]; then
    WIKI_CONTAINER=${a}
fi
