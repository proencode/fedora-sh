...
```
sudo dnf -y update
echo "🎵 --- ## 1. 도커 설치 --- 1. 시스템을 최신버전으로 유지하기 위해 다음을 실행한다."
```

2. 시스템 명령어인 dnf 의 플러그인 코어를 받는다.
```
sudo dnf -y install dnf-plugins-core
echo "🎵 --- ## 1. 도커 설치 --- 2. 시스템 명령어인 dnf 의 플러그인 코어를 받는다."
```
...
gpgkey=https://download.docker.com/linux/fedora/gpg
__EOF__
echo "📌 --- ## 1. 도커 설치 --- 3. 지금 받으려는 docker-ce 가 있는 위치를 리파지토리에 기록한다."
```
...
sudo dnf -y install docker-ce docker-ce-cli containerd.io
sudo systemctl enable --now docker
echo "📌 --- ## 1. 도커 설치 --- 4. docker-ce 를 받는다."
```
...
rpm -qi docker-compose
sudo docker ps -a
echo "📌 --- ## 1. 도커 설치 --- 6. docker-compose 를 설치한다."
```
...
echo "✍ (2-8) docker-compose.yml 이름을 지정 : [ 그냥 엔터 = ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
tempname="db_srv"
echo "✍ (3-1) 데이터베이스 서비스 이름 : [ 그냥 엔터 = ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
tempname="wikidb"
echo "✍ (3-3) 데이터베이스 이름 : [ 그냥 엔터 = ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
echo "✍ (3-5) wiki 데이터베이스 보관 디렉토리 : [ 그냥 엔터 = ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
tempname="pgwikic"
echo "✍ ---> (3-10) 데이터베이스 컨테이너 이름 : [ 그냥 엔터 = ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
tempname="imwiki"
echo "✍ (3-12) 데이터베이스 유저 이름 : [ 그냥 엔터 = ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
tempname="wikipswd"
echo "✍ (3-14) 데이터베이스 유저 비번 : [ 그냥 엔터 = ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
tempname="wiki_srv"
echo "✍ (4-1) 위키 서비스 이름 : [ 그냥 엔터 = ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
tempname="7700"
echo "✍ (4-3) 위키 포트 번호 : [ 그냥 엔터 = ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
tempname="wikijsc"
echo "✍ (4-5) 위키 컨테이너 이름 : [ 그냥 엔터 = ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    WIKI_CONTAINER=${a}
fi
...
echo "✍ (4-7) rclone 백업용 클라우드 이름 : [ 그냥 엔터 = ${tempname} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
echo "......(5-2) 데이터베이스 컨테이너 = ${DATABASE_CONTAINER}"
