...
echo "(4-11) cat <<__EOF__ | tee ${NAME_DEFINE_FILE}"
cat <<__EOF__ | tee ${NAME_DEFINE_FILE}
...
echo "🖍️ (5-2) 데이터베이스 컨테이너 이름: [ 그냥 엔터 = ${DATABASE_CONTAINER} ]"
read a
if [ "x${a}" != "x" ]; then
    tempname=${a}
fi
...
echo "🖍️ (5-4) 데이터베이스 유저 이름: [ 그냥 엔터 = ${DB_NAME} ]"
read a
if [ "x${a}" != "x" ]; then
    DB_NAME=${a}
fi
...
echo "🖍️ (5-6) rclone 용 클라우드 이름: [ 그냥 엔터 = ${CLOUD_NAME} ]"
read CLOUD_NAME
...
🖍️ ${bbb}(9-5)${xxx} 현재의 DB 를 백업하지 않으려면, 소문자 'n' 을 눌러 주세요."
__EOF__
    read n_is_no_backup ; echo "${uuu}"
    echo "[ ${n_is_no_backup} ]"
    if [ "x$n_is_no_backup" = 'xn' ]; then
