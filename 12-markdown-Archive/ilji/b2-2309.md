...
echo "#-- ${bbb}(9-2)${xxx} \${restore_sql_7z} = ${restore_sql_7z};"

if [ "x${restore_sql_7z}" = "x" ] || [ ! -f "${restore_sql_7z}" ]; then
    cat <<__EOF__
#-- ${bbb}(9-3)${xxx} ls -lR ${LOCAL_FOLDER} #--- 백업 디렉토리
$(ls -lR ${LOCAL_FOLDER})
#--
...
echo "#-- ${bbb}(9-4)${xxx} 현재의 DB 를 last_backup 으로 백업하지 않으려면, ' n ' 을 눌러 주세요."
read a ; echo "${uuu}"
echo "[ ${a} ]"
...
cat <<__EOF__
#-- ${bbb}(9-5)${xxx} 백업을 시작하기전 위키 컨테이너를 멈춥니다.
...
if [ "x${last_skip}" = "xdb_backup_ok" ]; then
  ...
  cat <<__EOF__
#-- ${bbb}(9-6)${xxx} 지정한 백업파일을 데이터베이스에 붓기 전에,
...
cat <<__EOF__
#-- ${bbb}(9-12)${xxx} 멈췄던 위키 컨테이너를 다시 시작합니다.
#-- sudo docker start ${WIKI_CONTAINER} ; sudo docker ps -a
__EOF__
sudo docker start ${WIKI_CONTAINER} ; sudo docker ps -a
echo "#-- ${bbb}(9-13)${xxx} 백업 작업이 끝났습니다."
```
