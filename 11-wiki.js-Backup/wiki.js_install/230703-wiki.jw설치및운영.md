...
### 5. `rclone` 프로그램은 구글 드라이브 뿐 아니라 다른 클라우드 스토리지도 연결할수 있으므로,

어느 클라우드를  쓸 것인지 선택해야 한다. 여기서는 구글 드라이브를 선택한다.
...
### (1) 색상을 표시하는 color_base 파일을 ~/bin 에 저장한다.
...
### (2) 색상 스크립트를 불러들이는 골격인 header 파일을 ~/bin 폴더에 저장한다.
...
source ${HOME}/bin/color_base #-- 221027목-1257 CMD_DIR CMD_NAME cmdRun cmdCont cmdYenter echoSeq 
MEMO="## 단순백업: wiki.js 백업파일을 홈 디렉토리에 보관하기 ##"
cat <<__EOF__
${cMagenta}>>>>>>>>>>${cGreen} $0 ${cMagenta}||| ${cCyan}${MEMO} ${cMagenta}>>>>>>>>>>${cReset}
__EOF__
# zz00logs_folder="${HOME}/zz00logs" ; if [ ! -d "${zz00logs_folder}" ]; then cmdRun "mkdir ${zz00logs_folder}" "로그 폴더" ; fi
...
LOCAL_FOLDER="${HOME}/wiki_backup" #- 보관용 로컬 저장소
...
# RCLONE_NAME="mygmail" #- rclone 이름
# REMOTE_FOLDER="wiki_backup" #-- 원격 저장소의 첫번째 폴더 이름
...
sudo docker exec wikijsdb pg_dumpall -U wikijs | 7za a -p -si ${LOCAL_Y2M2}/${DB_sql7z} ; echo "#-- (5) 오늘 요일이름의 로컬 보관장소에 백업합니다."
...
### restore_wikijs_from_backup_file.sh 
