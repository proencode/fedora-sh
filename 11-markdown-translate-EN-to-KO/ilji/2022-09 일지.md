...
1. gimp 프로그램에서, [Ctrl] + [Shift] + [`E`] 로 디스크로 내보내기 > 엑셀 G 칼럼에 만들어둔 `01-오남읍 양지로278길 35-3.png` 을 파일 이름으로 저장.
2. 여벌로 --> 파일 크기가 훨씬 작은 타입인 .webp 로 만들기 위해 "01-오남읍 양지로278길 35-3`.webp`" 를 파일 이름으로 한번더 저장.
