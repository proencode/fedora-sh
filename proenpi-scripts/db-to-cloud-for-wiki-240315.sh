...
#-- 2024/wolmal = 매월 마지막 백업 1개씩 12개 보관.
#-- 2024/jumal = 매주 마지막 백업 1개씩 53개 보간.
#-- 2024/ = 최근 1주일치만 7개 보관.

LOCAL_WOLMAL12=${LOCAL_FOLDER}/${this_year}/wolmal12 #-- 2024/wolmal = 매월말 백업 1개씩 12개 보관.
LOCAL_WEEK7=${LOCAL_FOLDER}/${this_year}/ #-- 2024/ = 최근 1주일치만 7개 보관.
LOCAL_JUMAL53=${LOCAL_FOLDER}/${this_year}/jumal53 #-- 2024/jumal = 매주말 백업 1개씩 53개 보간.

REMOTE_WOLMAL12=${REMOTE_FOLDER}/${this_year}/wolmal12 #-- 2024/wolmal12
REMOTE_WEEK7=${REMOTE_FOLDER}/${this_year}/ #-- 2024/
REMOTE_JUMAL53=${REMOTE_FOLDER}/${this_year}/jumal53 #-- 2024/jumal53
...
if [ ! -d ${LOCAL_WEEK7} ]; then
	showno="1a" ; showqq="보관용 로컬 디렉토리를 만듭니다."
	show_then_run "mkdir -p ${LOCAL_WEEK7} ; sudo chown ${USER}:${USER} ${LOCAL_WEEK7}"
	showno="1b" ; showqq="보관용 원격 디렉토리를 만듭니다."
	show_then_run "/usr/bin/rclone mkdir ${RCLONE_NAME}:${REMOTE_WEEK7}"
fi
if [ ! -d ${LOCAL_JUMAL53} ]; then
	showno="1c" ; showqq="보관용 로컬 디렉토리를 만듭니다."
	show_then_run "mkdir -p ${LOCAL_JUMAL53} ; sudo chown ${USER}:${USER} ${LOCAL_JUMAL53}"
	showno="1d" ; showqq="보관용 원격 디렉토리를 만듭니다."
	show_then_run "/usr/bin/rclone mkdir ${RCLONE_NAME}:${REMOTE_JUMAL53}"
fi
showno="2" ; showqq="보관용 로컬 디렉토리 입니다."
show_then_run "ls -lR ${LOCAL_WEEK7}"


showno="3" ; showqq="오늘날짜 클라우드 백업파일이 있는지 확인 합니다."
show_then_view "REMOTE_SQL_7Z_LIST=\$(/usr/bin/rclone ls ${RCLONE_NAME}:${REMOTE_WEEK7}/ | grep ${yoil_sql_7z} | awk '{print \$2}')"
REMOTE_SQL_7Z_LIST=$(/usr/bin/rclone ls ${RCLONE_NAME}:${REMOTE_WEEK7}/ | grep ${yoil_sql_7z} | awk '{print $2}')
...
if [ "x$REMOTE_SQL_7Z_LIST" != "x" ]; then
	...
	do
		...
		OUTRC=$(/usr/bin/rclone deletefile ${RCLONE_NAME}:${REMOTE_WEEK7}/${file_name})
		showno="4a2" ; showqq="오늘날짜 클라우드 백업파일을 삭제합니다."
		show_then_view "OUTRC=\$(/usr/bin/rclone deletefile ${RCLONE_NAME}:${REMOTE_WEEK7}/${file_name}) ${cMagenta}#----${cYellow}${OUTRC}${cMagenta}----"
		show_then_view "#"
	done
else
...
show_then_run "rm -f ${LOCAL_WEEK7}/*${yoil_sql_7z}"
...
if [ "x${DB_TYPE}" = "xmysql" ]; then
	show_then_run "/usr/bin/mysqldump --login-path=${LOGIN_PATH} --column-statistics=0 ${DB_NAME} | 7za a -mx=9 -si ${LOCAL_WEEK7}/${YOIL_sql7z} -p${PSWD_GEN_CODE}"
else
if [ "x${DB_TYPE}" = "xpgsql" ]; then
	show_then_run "sudo docker exec wikijsdb pg_dumpall -U wikijs | 7za a -mx=9 -si ${LOCAL_WEEK7}/${YOIL_sql7z} -p${PSWD_GEN_CODE}"
else
	cat <<__EOF__
...
OUTRC=$(/usr/bin/rclone copy ${LOCAL_WEEK7}/${YOIL_sql7z} ${RCLONE_NAME}:${REMOTE_WEEK7}/)
showno="7" ; showqq="로컬 DB 백업파일을 클라우드로 복사합니다."
show_then_view "OUTRC=\$(/usr/bin/rclone copy ${LOCAL_WEEK7}/${YOIL_sql7z} ${RCLONE_NAME}:${REMOTE_WEEK7}/) ${cMagenta}#----${cYellow}${OUTRC}${cMagenta}----"


showno="8" ; showqq="${this_year}/${this_wol} 최근 일주일 백업을 끝냅니다. (${ymd_hm})"
show_then_view "#"
...
show_title "${REMOTE_WEEK7} 월의 마지막 백업파일을 ${REMOTE_WOLMAL12} 년도로 복사 시작 (${ymd_hm})"


showno="9" ; showqq="${this_wol}월 백업파일이 이전에 백업돼 있었는지 확인 합니다."
show_then_view "REMOTE_SQL_7Z_LIST=\$(/usr/bin/rclone ls ${RCLONE_NAME}:${REMOTE_WOLMAL12}/ | grep ${this_wol_sql_7z} | awk '{print \$2}')"
REMOTE_SQL_7Z_LIST=$(/usr/bin/rclone ls ${RCLONE_NAME}:${REMOTE_WOLMAL12}/ | grep ${this_wol_sql_7z} | awk '{print $2}')
...
if [ "x$REMOTE_SQL_7Z_LIST" != "x" ]; then
	...
	do
		...
		OUTRC=$(/usr/bin/rclone deletefile ${RCLONE_NAME}:${REMOTE_WOLMAL12}/${file_name})
...
show_then_run "rm -f ${LOCAL_WOLMAL12}/*${this_wol_sql_7z}"



showno="12" ; showqq="${REMOTE_WEEK7} 월 백업파일을 ${REMOTE_WOLMAL12} 년도로 복사하는 작업을 시작합니다. (${ymd_hm})"
show_then_view "#"


showno="13" ; showqq="로컬 디렉토리의 월 백업파일을 년도로 복사합니다."
show_then_run "cp ${LOCAL_WEEK7}/${YOIL_sql7z} ${LOCAL_WOLMAL12}/${WOL_sql7z}"

OUTRC=$(/usr/bin/rclone copy ${LOCAL_WOLMAL12}/${WOL_sql7z} ${RCLONE_NAME}:${REMOTE_WOLMAL12}/)
showno="14" ; showqq="${this_wol}월 백업파일을 ${this_year}년도 폴더로 복사합니다."
show_then_view "OUTRC=\$(/usr/bin/rclone copy ${LOCAL_WOLMAL12}/${WOL_sql7z} ${RCLONE_NAME}:${REMOTE_WOLMAL12}/) ${cMagenta}#----${cYellow}${OUTRC}${cMagenta}----"

OUTRC=$(/usr/bin/rclone ls ${RCLONE_NAME}:${REMOTE_WOLMAL12})
showno="15" ; showqq="폴더 확인"
show_then_view "OUTRC=\$(/usr/bin/rclone ls ${RCLONE_NAME}:${REMOTE_WOLMAL12}) ${cMagenta}#----${cYellow}${OUTRC}${cMagenta}----"

showno="16" ; showqq="${REMOTE_WEEK7} 월 백업파일을 ${REMOTE_WOLMAL12} 년도로 복사하는 작업을 끝냅니다. (${ymd_hm})"
show_then_view "#"
...
show_title "${REMOTE_WEEK7} 월의 마지막 백업파일을 ${REMOTE_JUMAL53} 폴더에 J${ju_beonho} 번호로 복사 시작 (${ymd_hm})"


showno="17" ; showqq="${this_wol}월 백업파일이 이전에 백업돼 있었는지 확인 합니다."
show_then_view "REMOTE_SQL_7Z_LIST=\$(/usr/bin/rclone ls ${RCLONE_NAME}:${REMOTE_JUMAL53}/ | grep ${ju_beonho_sql_7z} | awk '{print \$2}')"
REMOTE_SQL_7Z_LIST=$(/usr/bin/rclone ls ${RCLONE_NAME}:${REMOTE_JUMAL53}/ | grep ${ju_beonho_sql_7z} | awk '{print $2}')
...
if [ "x$REMOTE_SQL_7Z_LIST" != "x" ]; then
	...
	do
		...
		OUTRC=$(/usr/bin/rclone deletefile ${RCLONE_NAME}:${REMOTE_JUMAL53}/${file_name})
		showno="18a2" ; showqq="${this_wol}월 백업파일을 삭제합니다."
		show_then_view "OUTRC=\$(/usr/bin/rclone deletefile ${RCLONE_NAME}:${REMOTE_JUMAL53}/${file_name}) ${cMagenta}#----${cYellow}${OUTRC}${cMagenta}----"
	done
else
...
show_then_run "rm -f ${LOCAL_JUMAL53}/*${ju_beonho_sql_7z}"


showno="20" ; showqq="${ju_beonho_sql_7z} 백업파일을 ${REMOTE_JUMAL53} 로 복사하는 작업을 시작합니다. (${ymd_hm})"
show_then_run "cp ${LOCAL_WEEK7}/${YOIL_sql7z} ${LOCAL_JUMAL53}/${JU_sql7z}"

OUTRC=$(/usr/bin/rclone copy ${LOCAL_JUMAL53}/${JU_sql7z} ${RCLONE_NAME}:${REMOTE_JUMAL53}/)
showno="21" ; showqq="${this_wol}월 백업파일을 ${REMOTE_JUMAL53} 폴더로 복사합니다."
show_then_view "OUTRC=\$(/usr/bin/rclone copy ${LOCAL_JUMAL53}/${JU_sql7z} ${RCLONE_NAME}:${REMOTE_JUMAL53}/) ${cMagenta}#----${cYellow}${OUTRC}${cMagenta}----"

showno="22a" ; showqq="보관용 로컬 디렉토리 입니다."
show_then_run "ls -lR ${LOCAL_WOLMAL12}"
showno="22b" ; showqq="원격 디렉토리 입니다."
show_then_run "/usr/bin/rclone lsl ${RCLONE_NAME}:${REMOTE_WOLMAL12}"

showno="23" ; showqq="${REMOTE_YOIL} 월의 마지막 백업파일을 ${REMOTE_JUMAL53} 폴더에 J${ju_beonho} 번호로 복사하는 작업을 끝냅니다. (${ymd_hm})"
show_then_view "#"
