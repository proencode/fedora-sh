...
# |
# | time sudo bash -c "docker exec ${DB_CONTAINER} pg_dumpall -U ${DB_USER} | 7za a -mx=9 -p${BACKUP_PSWD} -si ${BACKUP_FILE}" # OK
__EOF__

    time sudo bash -c "docker exec ${DB_CONTAINER} pg_dumpall -U ${DB_USER} | 7za a -mx=9 -p${BACKUP_PSWD} -si ${BACKUP_FILE}"
