...
DB_NAME=$(enter_value "${qq}-4. pgsql 데이터베이스 이름:" "wikidb")
echo "${rrr}[ ${xxx}${DB_NAME} ${rrr}]${xxx}"
DB_USER=$(enter_value "${qq}-5. pgsql 사용자 이름:" "wikiuser")
echo "${rrr}[ ${xxx}${DB_USER} ${rrr}]${xxx}"
DB_PASS=$(enter_passwd "${qq}-6. pgsql 비번:" "wikipswd")
echo "........"
echo "password"
###- echo "${rrr}[ ${xxx}${DB_PASS} ${rrr}]${xxx}"
DB_CONTAINER=$(enter_value "${qq}-7. 데이터베이스 컨테이너 이름:" "dbcon")
echo "${rrr}[ ${xxx}${DB_CONTAINER} ${rrr}]${xxx}"
WIKI_PORT=$(enter_value "${qq}-8. 위키연결 포트번호:" "5940")
echo "${rrr}[ ${xxx}${WIKI_PORT} ${rrr}]${xxx}"
WIKI_CONTAINER=$(enter_value "${qq}-9. 위키 컨테이너:" "wikicon")
...
__EOF__
    ...
    cmdrun "time sudo bash -c \"docker exec ${DB_CONTAINER} pg_dumpall -U ${DB_USER} | 7za a -mx=9 -p${BACKUP_PSWD} -si ${BACKUP_FILE}\"" "(5-2) 업로드 하기전에, 현재의 wiki DB 파일을 백업합니다."

    cmdrun "time rclone copy ${BACKUP_FILE} ${CLOUD_NAME}:${CLOUD_Y2M2}/" "(5-3) wiki DB 파일을 클라우드로 복사합니다."
    cmdrun "time rclone lsl ${CLOUD_NAME}:${CLOUD_Y2M2}" "(5-4) 클라우드 폴더의 내용입니다."
    #--
    #-- <---- 현재 wiki DB 를 다운로드 + 백업 합니다.
fi

cmdrun "time sudo docker exec -it ${DB_CONTAINER} dropdb -U ${DB_USER} ${DB_NAME}" "(6) 현재의 wiki DB 를 삭제합니다."

cmdrun "time sudo docker exec -it ${DB_CONTAINER} createdb -U ${DB_USER} ${DB_NAME}" "(7) 빈 wiki DB 를 새로 만듭니다."
...
cmdrun "time 7za x -p${backup_pswd} -so ${RESTORE_FILE} | sudo docker exec -i ${DB_CONTAINER} psql -U ${DB_USER} -d ${DB_NAME}" "(8) 백업된 wiki DB 파일을 데이터베이스에 다시 붓습니다. (RESTORE)"
